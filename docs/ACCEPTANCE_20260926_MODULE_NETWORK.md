...
首轮 CI 虽通过，但检查实际附件时发现 MSIX 文件名与包标识仍为 0.1.6.0。新增打包回归先以 `packaging:test_artifact_version_mismatch` 失败；打包与校验脚本现读取 Cargo 版本，manifest 与 Cargo 不一致则在打包前拒绝，manifest 更新为 0.1.7.0。

修正只涉及打包、发布工作流与文档，运行时代码和桌面独立版保持不变。工作流仅允许这种受限的打包/文档更正更新同版本附件，不重写公开标签；运行时或依赖有变化必须增加版本。发布前继续核验实际附件版本，不能只看 CI 成功。
