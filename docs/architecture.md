...
**Attribution vs. interpretation on firmware-defined reads.** Fail-closed means never claiming a value the bytes do not support — not discarding a real answer because one line did not fit a model. On an optional read-only probe the streaming parser (`crates/at-protocol/src/parser.rs`) therefore decides *attribution* only: a line carrying the command's own response prefix is collected, and any other non-final text line is collected as an unattributable free-form line under `MAX_UNRECOGNIZED_LINES` instead of aborting the transaction. Interpreting the collected lines stays with the command's own parser, which skips what it cannot read rather than guessing. `AT+QTEMP` is the worked example: the DJI Gen-1 module answers `+QTEMP: 57,51,51` — an unnamed positional list, not the `"name",value` form other firmwares use — the parser reads the values in report order without inventing channel names, and the raw line is kept beside the reading (`FeatureProbeState::temperature_raw`, UI-local, never serialized) so an unreadable layout is still visible to the user. Wire-level faults — NMEA sentences, binary noise, oversized lines, a missing final code, malformed SMS PDUs — keep failing closed, and a probe with no readable value is reported as `FormatMismatch`/`Empty` rather than as a plausible-looking number.
